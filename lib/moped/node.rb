...
module Moped
  ...
  class Node
    ...
    def connection
      begin
        pool.with do |conn|
          Moped.logger.debug("MOPED: Starting connection:#{conn.object_id.to_s(36)} in thread:#{Thread.current.object_id.to_s(36)}")
          yield(conn)
          Moped.logger.debug("MOPED: Ending connection:#{conn.object_id.to_s(36)} in thread:#{Thread.current.object_id.to_s(36)}")
        end
